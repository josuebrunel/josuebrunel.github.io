...
## How Go generated a fake population

The generator from [last post]({{< ref "generating-synthetic-oscar-patients-without-synthea.md" >}}) inherited its shape from [the CSV load 2 posts ago]({{< ref "oscar-emr-ships-empty.md" >}}): a hand-rolled worker pool, `sync` and `chan` from the standard library, each patient written in its own transaction, sized for the job instead of pulling in a library.

Sizing it correctly meant catching the bug that only shows up once you go concurrent: a random number generator shared across 16 goroutines is a data race, full stop. The fix was one line per worker, its own generator, seeded from the base seed plus its own worker ID, so a debug run stays reproducible and a production run stays race-free at the same time. It's the kind of bug that never shows up in a single-threaded test and always shows up the first time someone actually needs the speedup.

The payoff: 10,000 patients, their full health records along with them, generated and ingested in 1m14s, zero failures. The older CSV load was a different workload, 18 minutes serial down to 10 fresh with the pool, 3.5 on rerun, that last bit mostly the idempotent hashing rather than the pool. That's the embarrassingly parallel half of this story: independent patients, fanned out over goroutines, gathered back through a channel. The dashboard reading that data back out doesn't have that problem. It has a different one.
